...
def test_addition(
    manifold: Manifold, tolerance: Tuple[float, float], uniform_points: torch.Tensor
) -> None:
    ...
    # Additive inverse
    torch.testing.assert_close(
        manifold.addition(-uniform_points, uniform_points) + 1,
        identity + 1,
        atol=atol,
        rtol=rtol,
    )
    ...
    # Distributive law
    torch.testing.assert_close(
        -manifold.addition(x, y), manifold.addition(-x, -y), atol=atol, rtol=rtol
    )
    # Gyrotriangle inequality
    assert torch.all(
        manifold.addition(x, y).norm(p=2, dim=-1, keepdim=True)
        <= manifold.addition(
            x.norm(p=2, dim=-1, keepdim=True), y.norm(p=2, dim=-1, keepdim=True)
        )
        + atol
    )
...
def test_scalar_mul(
    ...
    if isinstance(manifold, (Euclidean, PoincareBall)):
        ...
        torch.testing.assert_close(
            n_sum,
            manifold.scalar_mul(n * identity, uniform_points),
            atol=atol,
            rtol=rtol,
        )
        # Distributive laws
        ...
        left_side /= manifold.scalar_mul(r1, uniform_points).norm(
            p=2, dim=-1, keepdim=True
        )
        torch.testing.assert_close(
            left_side,
            uniform_points / uniform_points.norm(p=2, dim=-1, keepdim=True),
            atol=atol,
            rtol=rtol,
        )
        # Homogenity property
        torch.testing.assert_close(
            ...
            manifold.scalar_mul(
                torch.abs(r1), uniform_points.norm(p=2, dim=-1, keepdim=True)
            ),
            atol=atol,
            rtol=rtol,
        )
    ...
    torch.testing.assert_close(
        res[0, 1:], torch.zeros_like(res[0, 1:]), atol=atol, rtol=rtol
    )
    # Stability of multiplication with large scalars
    if (
        isinstance(manifold, (Euclidean, PoincareBall))
        or uniform_points.dtype == torch.float64
    ):
...
def test_dist(
    ...
    if isinstance(manifold, Hyperboloid):
        origin = manifold._create_origin_from_reference(uniform_points)
    elif isinstance(manifold, PoincareBall) or isinstance(manifold, Euclidean):
        origin = torch.zeros_like(uniform_points)
    ...
    # Symmetry
    torch.testing.assert_close(
        manifold.dist(x, y), manifold.dist(y, x), atol=atol, rtol=rtol
    )
    # Triangle inequality
    assert torch.all(
        manifold.dist(x, z) <= manifold.dist(x, y) + manifold.dist(y, z) + atol
    )
...
def test_expmap_retraction_logmap(
    manifold: Manifold, tolerance: Tuple[float, float], uniform_points: torch.Tensor
) -> None:
    ...
    torch.testing.assert_close(
        manifold.expmap(v0, origin), manifold.expmap_0(v0), atol=atol, rtol=rtol
    )
    torch.testing.assert_close(
...
def test_ptransp(
    ...
    torch.testing.assert_close(
        manifold.ptransp(u, origin, uniform_points), u_pt, atol=atol, rtol=rtol
    )
    # Numerical stability
    torch.testing.assert_close(
        manifold.ptransp(u_pt, uniform_points, origin), u, atol=atol, rtol=rtol
    )
    assert manifold.is_in_tangent_space(
        manifold.ptransp(u_pt, uniform_points, origin), origin
    )
...
def test_gyration(
    ...
    torch.testing.assert_close(
        manifold._gyration(x, torch.zeros_like(x), z), z, atol=atol, rtol=rtol
    )
    torch.testing.assert_close(
        manifold._gyration(torch.zeros_like(x), x, z), z, atol=atol, rtol=rtol
    )
    torch.testing.assert_close(manifold._gyration(x, x, z), z, atol=atol, rtol=rtol)
    torch.testing.assert_close(
        manifold._gyration(x, y, torch.zeros_like(x)),
        torch.zeros_like(x),
        atol=atol,
        rtol=rtol,
    )
    torch.testing.assert_close(
        manifold._gyration(x, y, -z), -manifold._gyration(x, y, z), atol=atol, rtol=rtol
    )
