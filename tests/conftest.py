...
@pytest.fixture(
    scope="package",
    params=[Euclidean, Hyperboloid, PoincareBall],
    ids=["Euclidean", "Hyperboloid", "PoincareBall"],
)
...
def uniform_points(
    seed: None, manifold: Manifold, request: pytest.FixtureRequest
) -> torch.Tensor:
    ...
    if isinstance(manifold, Euclidean):
        bound = 100
        points = torch.empty((num_pts, dim), dtype=manifold.dtype).uniform_(
            -bound, bound
        )
