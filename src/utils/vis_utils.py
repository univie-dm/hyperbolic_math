...
def create_figure(
    ...
    poincare = PoincareBall(
        c=_manifold.c.to(settings["plot_manifold_dtype"]),
        dtype=settings["plot_manifold_dtype"],
    )
    ...
    if isinstance(_manifold, PoincareBall) and points.shape[-1] > 2:
        ...
        if hyperplanes is not None:
            hyperplanes = (
                poincare.to_hyperboloid(hyperplanes[0]),
                poincare.to_hyperboloid(hyperplanes[1]),
            )
        points, hyperplanes = pointsTo2dPoincare(
            points, poincare, hyperplanes, settings
        )
        ...
        hyperplanes = (
            (hyperplanes[0], hyperplanes[1]) if hyperplanes is not None else None
        )
    ...
    if handles:
        ax.legend(
            handles=handles, bbox_to_anchor=(1.05, 1), loc="upper left", borderaxespad=0
        )
...
def pointsTo2dPoincare(
    ...
    if settings["dim_red_method"] == "HoroPCA":
        ...
        raise ValueError(
            f"Unknown dimensionality reduction method {settings['dim_red_method']}"
        )
...
def plot_2d_points(
    x: torch.Tensor, ax: plt.Axes, labels: Union[npt.ArrayLike, None] = None
) -> List[plt.Line2D]:
...
def plot_edges(
    ...
    assert len(edges[0]) == len(edges[0]), (
        "Start and end points must have the same shape"
    )
    spacing = 100
...
def plot_hyperplane(
    hyperplanes: Tuple[torch.Tensor, torch.Tensor],
    poincare: PoincareBall,
    ax: plt.Axes,
    handles: List[plt.Line2D],
) -> None:
    ...
    ax.scatter(
        hyperplane_base_points[:, 0],
        hyperplane_base_points[:, 1],
        c="green",
        marker="P",
        s=50,
        zorder=4,
