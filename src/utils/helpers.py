...
def compute_pairwise_distances(
    ...
    distmat = torch.zeros((points.shape[0], points.shape[0]), dtype=manifold.dtype).to(
        device
    )
    indices = torch.triu_indices(points.shape[0], points.shape[0], 1).to(device)
    while indices.shape[1] > 0:
        dist_batch = manifold.dist(
            points[indices[0, :batch_size]],
            points[indices[1, :batch_size]],
            version=version,
        ).reshape(-1)
...
def get_delta(
    points: torch.Tensor, manifold: Manifold, sample_size=1500, version="average"
):
    """
...
def compute_hyperbolic_delta(distmat: torch.Tensor, version: str) -> torch.Tensor:
    ...
    max_min_prod = (
        torch.min(gromov_prod_mat.unsqueeze(1), gromov_prod_mat.unsqueeze(0))
        .max(dim=2)
        .values
    )
