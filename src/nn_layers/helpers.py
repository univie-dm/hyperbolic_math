...
def compute_mlr_Hyperboloid(
    ...
    z_norm = (
        z.norm(p=2, dim=hyperbolic_axis, keepdim=True).clamp_min(manifold.min_enorm).T
    )  # (1, out_dim)
    ...
    eps = (
        ...
        if manifold.dtype == torch.float32
...
def compute_mlr_PoincarePP(
    ...
    z_norm = z.norm(p=2, dim=hyperbolic_axis, keepdim=True).clamp_min(
        manifold.min_enorm
    )  # (out_dim, 1)
    lambda_x = manifold._lambda(x, axis=hyperbolic_axis)  # (B, 1)
    z_unitx = (x.unsqueeze(-1) * (z / z_norm).T).sum(dim=1)  # (B, out_dim)
    asinh_arg = (1 - lambda_x) * sinh(sqrt_c2r) + sqrt_c * lambda_x * cosh(
        sqrt_c2r
    ) * z_unitx  # (B, out_dim)
    # Improve the performance by smoothly clamping the input of asinh() to approximately the range of ...
    # ... [-16*clamping_factor, 16*clamping_factor] for float32
