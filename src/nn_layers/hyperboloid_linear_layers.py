...
class HyperbolicLinearHyperboloid(torch.nn.Module):
    ...
    def __init__(
        ...
        assert isinstance(manifold, Hyperboloid), (
            "manifold must be an instance of Hyperboloid"
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.input_space == "manifold":
            ...
            assert self.manifold.is_in_tangent_space(
                x,
                self.manifold._create_origin_from_reference(
                    x, axis=self.hyperbolic_axis
                ),
                axis=self.hyperbolic_axis,
            )

        # Matrix-Vector multiplication in the tangent space at the Hyperboloid origin
        x_rem = x.narrow(
            self.hyperbolic_axis, 1, x.shape[self.hyperbolic_axis] - 1
        )  # (B, in_dim - 1)
        x = (x_rem.unsqueeze(-1) * self.weight.T.unsqueeze(0)).sum(
            dim=1
        )  # (B, out_dim - 1)
        ...
        # Bias addition
        bias = torch.cat(
            [torch.zeros_like(self.bias[:, :1]), self.bias], dim=1
        )  # (1, out_dim)
        pt_bias = self.manifold.ptransp_0(
            bias, x, axis=self.hyperbolic_axis
        )  # (1, out_dim)
        res = self.manifold.expmap(
            pt_bias, x, axis=self.hyperbolic_axis
        )  # (B, out_dim)
        return res
...
class HyperbolicLinearHyperboloidFHNN(torch.nn.Module):
    ...
    def __init__(
        ...
        bound = 0.02
        weight = torch.empty((output_dim, input_dim), dtype=self.params_dtype).uniform_(
            -bound, bound
        )
        weight[:, 0] = (
            0.0  # FHNN initializes the weights as tangent vectors w.r.t. the Hyperboloid origin
        )
        self.weight = torch.nn.Parameter(weight, requires_grad=requires_grad)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.dropout is not None:
            x = self.dropout(x)
        x = (x.unsqueeze(-1) * self.weight.T.unsqueeze(0)).sum(
            dim=1
        ) + self.bias  # (B, out_dim)
        ...
        res0 = (
            self.scale.exp() * x0.sigmoid() + 1 / self.manifold.c.sqrt() + self.eps
        )  # (B, 1)
        scale = (res0.pow(2) - 1 / self.manifold.c).sqrt() / x_rem.norm(
            p=2, dim=self.hyperbolic_axis, keepdim=True
        )  # (B, 1)
...
class HyperbolicLinearHyperboloidFHCNN(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.normalize:
            ...
            if mask.any():
                origin = self.manifold._create_origin_from_reference(
                    res, axis=self.hyperbolic_axis
                )
                res = torch.where(mask, origin, res)  # (B, out_dim)
        else:
            # Compute the time component from the space component and concatenate
            res0 = (
                x_rem.pow(2).sum(dim=self.hyperbolic_axis, keepdim=True)
                + 1 / self.manifold.c
            ).sqrt()  # (B, 1)
