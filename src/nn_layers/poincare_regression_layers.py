...
class HyperbolicRegressionPoincare(torch.nn.Module):
    ...
    def _compute_mlr(
        ...
        sub = self.manifold.addition(
            -p.T.unsqueeze(0), x.unsqueeze(-1), axis=1
        )  # (B, in_dim, out_dim)
        suba = (sub * a.T).sum(dim=1, keepdim=True)  # (B, 1, out_dim)
        a_norm = (
            a.norm(p=2, dim=self.hyperbolic_axis, keepdim=True)
            .clamp_min(self.manifold.min_enorm)
            .T
        )  # (1, out_dim)
        asinh_arg = (
            sqrt_c * self.manifold._lambda(sub, axis=1) * suba / a_norm
        )  # (B, 1, out_dim)
        ...
        eps = (
            ...
            if self.manifold.dtype == torch.float32
            else torch.finfo(torch.float64).eps
        )
        clamp = self.clamping_factor * float(math.log(2 / eps))
        asinh_arg = smooth_clamp(
            asinh_arg, -clamp, clamp, self.smoothing_factor
        )  # (B, out_dim)
        signed_dist2hyp = asinh(asinh_arg) / sqrt_c  # (B, 1, out_dim)
        res = (
            self.manifold._lambda(p, axis=self.hyperbolic_axis).T
            * a_norm
            * signed_dist2hyp.squeeze(1)
        )  # (B, out_dim)
        return res
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        pt_weight = self.manifold.ptransp_0(
            self.weight, self.bias, axis=self.hyperbolic_axis
        )
...
class HyperbolicRegressionPoincarePP(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        res = compute_mlr_PoincarePP(
            self.manifold,
            x,
            self.weight,
            self.bias,
            self.hyperbolic_axis,
